...
function renderGroupDoc(node, ctx) {
  const e = entry(ctx, node);
  // not cached for export, nor in interactive mode (children may be rendered at reduced detail then)
  const key = (ctx.full || ctx.fast) ? null : nodeStateKey(node, ctx) + '|' + ctx.scale + '|' + ctx.W + 'x' + ctx.H;
...
function maskCoverage(node, ctx) {
  const e = entry(ctx, node);
  const key = (ctx.full || ctx.fast) ? null : nodeStateKey(node, ctx) + '|' + ctx.scale + '|' + ctx.W + 'x' + ctx.H + '|' + (node.params.invert ? 1 : 0);
